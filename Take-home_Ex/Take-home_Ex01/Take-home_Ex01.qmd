...
Where is the missing value ??!! We found 47 missing values in the ESCS column.
...
p2 <- ggplot(stu_qqq_SG_01, aes(x = MeanOverall)) +
  geom_point(aes(y = MeanMATH), color = 'salmon',alpha = 0.5) +
  geom_smooth(aes(y = MeanMATH), method = 'lm', color = 'salmon') +
  geom_point(aes(y = MeanREAD), color = 'steelblue1',alpha = 0.5) +
  geom_smooth(aes(y = MeanREAD), method = 'lm', color = 'steelblue1') +
  geom_point(aes(y = MeanSCIE), color = 'darkseagreen',alpha = 0.5) +
  geom_smooth(aes(y = MeanSCIE), method = 'lm', color = 'darkseagreen') +
  theme_minimal() +
...
After categorizing by school, the scores in different subjects and the overall average scores of schools are dispersed. There is no clear case where any school performs well in every subject or performs poorly in every subject. The circles of four different colors indicate that there is no concentration at high or low scores.
...
There are some outliers, but not many, and the performance of students from most schools is relatively stable, ranging between 500-600. Regarding these outliers, I am interested in knowing if the outliers in each subject all come from the same schools.
...
The performance of students across subjects in Singapore schools is relatively even, with no apparent "extreme schools." Outliers come from various schools, each with its strengths and weaknesses, falling within a normal and reasonable range of consideration.
:::
...
ggplot(stu_qqq_SG_01, aes(x = ESCS)) +
  geom_point(aes(y = MeanOverall), color = 'lightseagreen',alpha = 0.5) +
  geom_smooth(aes(y = MeanOverall), method = 'lm', color = 'lightseagreen') +
  geom_point(aes(y = MeanSCIE), color = 'palevioletred1',alpha = 0.5) +
  geom_smooth(aes(y = MeanSCIE), method = 'lm', color = 'palevioletred1') +
  theme_minimal() +
...
-   Is there a correlation in the distribution of Singaporean students' academic performance across subjects? Singaporean students' academic scores show a high correlation with overall performance. Schools that excel in one subject typically also excel in other subjects and overall. However, in my analysis, I did not conduct further tests. The correlation is evident from the visualization, and further validation can be done if necessary.

-   Is there a relationship between academic performance and gender? Indeed, there are differences in performance across subjects between male and female students. For example, boys perform better in Mathematics and Science, while girls excel in Reading. However, on average, girls show more stable overall performance. But overall, the performance between boys and girls tends to be balanced.

-   Is there a difference in performance between different schools? There are some differences in the performance levels of different schools, but it's not extreme. Each school has students who perform at high levels and those who perform less well, but there's no absolute good or bad.

-   Is there a relationship between academic performance and socioeconomic status (ESCS)? Yes, there is a significant positive correlation, indicating that students with a higher socioeconomic status perform better in Mathematics, Reading, and Science. Regression analysis shows that ESCS is an important variable in predicting academic performance, significantly impacting overall student achievement. Therefore, consideration can be given to assisting students from lower socioeconomic backgrounds.
